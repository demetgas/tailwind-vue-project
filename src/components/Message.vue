<template>
  <div id="contact" class="max-w-[1140px] m-auto mt-[5svh]">
    <h2 class="text-center text-gray-700 p-4 font-bold text-4xl">
      Send us a message
    </h2>
    <p class="text-center text-gray-700 text-2xl">
      Can't wait to hear your thoughts!
    </p>
    <div class="grid md:grid-cols-2 mt-16">
      <img
        :src="f2"
        alt="/"
        class="md:h-full sm:w-[400px] sm:h-[400px] sm:mx-auto p-2 max-h-[500px] h-[200px] w-[420px] hover:scale-110 transition-all delay-75"
      />
      <form>
        <div class="grid grid-cols-2">
          <input
            class="border m-2 p-2 rounded"
            type="text"
            placeholder="Name"
          />
          <input
            class="border m-2 p-2 rounded"
            type="text"
            placeholder="Last Name"
          />
          <input
            class="border m-2 p-2 rounded"
            type="email"
            placeholder="Email"
          />
          <input
            class="border m-2 p-2 rounded"
            type="tel"
            placeholder="Phone"
          />
          <input
            class="border col-span-2 p-2 m-2 rounded"
            type="text"
            placeholder="Address"
          />
          <textarea
            class="border col-span-2 m-2 p-2 rounded"
            cols="30"
            rows="10"
          ></textarea>
          <button
            class="m-2 p-2 col-span-2 rounded text-white bg-hero2 hover:bg-hero"
          >
            Submit
          </button>
        </div>
      </form>
    </div>
  </div>
</template>
<script>
import f2 from "../assets/f2.jpeg";
export default {
  name: "MessageSection",
  data() {
    return {
      f2: f2,
    };
  },
};
</script>
